...
class CreateFinalDesignConfirmationTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('t_final_design_confirmations', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->unsignedBigInteger('lp_order_id');
            $table->string('image_path')->comment('画像パス')->nullable();
            $table->text('design_memo')->comment('制作メモ')->nullable();
            $table->text('law_support_memo')->comment('薬機修正内容')->nullable();
            $table->text('info_manage_memo')->comment('情報管理メモ')->nullable();
    ...
    {
        Schema::dropIfExists('t_final_design_confirmations');
    }
}
