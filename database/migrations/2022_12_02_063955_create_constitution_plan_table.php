...
class CreateConstitutionPlanTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('t_constitution_plans', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->unsignedBigInteger('lp_order_id')->comment('構成の手順ID');
            $table->string('block_detail')->comment('ブロック内容')->nullable();
            $table->string('requester_fix')->comment('依頼者修正内容')->nullable();
            $table->string('pharmaceutical_affairs_fix')->comment('薬事修正内容')->nullable();
            $table->string('information_management_memo')->comment('情報管理メモ')->nullable();
    ...
    {
        Schema::dropIfExists('t_constitution_plans');
    }
}
