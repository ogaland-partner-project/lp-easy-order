...
class CopyTest extends TestCase
{
    ...
    const API_URL = '/api/lp_easy_order/basic_knowledge/copy';
    ...
    public function test_正常()
    {
        ...
        $beforeBasicKnowledges = TBasicKnowledge::where('lp_order_id', $param['lp_order_id'])
            ->orderBy('id')->get();
        $beforeBasicKnowledgeDetails = $beforeBasicKnowledges->map(function ($basicKnowledge) {
            return [$basicKnowledge->id => $basicKnowledge->basicKnowledgeDetails];
        });
        ...
        $basicKnowledges->each(function ($basicKnowledge, $i) use ($param, $otherBasicKnowledges) {
            ...
            $this->assertArrayEquals($fromBasicKnowledge, $basicKnowledge, ['id']);
            // 基礎知識詳細
            // - コピー元とコピー先の個数が一致すること
