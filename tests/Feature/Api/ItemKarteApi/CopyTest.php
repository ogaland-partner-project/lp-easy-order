...
class CopyTest extends TestCase
{
    ...
    const API_URL = '/api/lp_easy_order/item_karte/copy';
    ...
    public function test_正常()
    {
        ...
        $beforeItemKartes = TItemKarte::where('lp_order_id', $param['lp_order_id'])
            ->orderBy('id')->get();
        $beforeRawMaterials = $beforeItemKartes->map(function ($itemKarte) {
            return [$itemKarte->id => $itemKarte->rawMaterials];
        });
        ...
        // 実行前にコピー先に紐付いていた消費カルテ情報が削除されていること
        ...
        $itemKartes->each(function ($itemKarte, $i) use ($param, $otherItemKartes) {
            ...
            $this->assertArrayEquals($fromItemKarte, $itemKarte, ['id']);
            ...
            $itemKarte->rawMaterials->each(function ($rawMaterial, $i) use ($fromItemKarte) {
                ...
                $fromRawMaterial->item_karte_id = $rawMaterial->item_karte_id;
                $this->assertArrayEquals($fromRawMaterial, $rawMaterial, ['id']);
            });
        });
    }
     ...
     * @return void
     */
    public function test_異常：コピー先LP構成IDなし()
    {
        $param = [
            'other_lp_order_id' => 2001,
        ];
     ...
     * @return void
     */
    public function test_異常：コピー先LP構成IDが非数値()
    {
        $param = [
            'lp_order_id' => '１００１',
            'other_lp_order_id' => 2001,
     ...
     * @return void
     */
    public function test_異常：コピー元LP構成IDなし()
    {
        $param = [
            'lp_order_id' => 1001,
        ];
        ...
        $response->assertStatus(422)
            ->assertJson([
                'dataArray' => [],
                'normalMessage' => '',
                'errorMessage' => '複製元LP構成IDは必ず指定してください。',
            ]);
    }
     ...
     * @return void
     */
    public function test_異常：コピー元LP構成IDが非数値()
    {
        $param = [
            'lp_order_id' => 1001,
            'other_lp_order_id' => '２００１',
        ];
        ...
        $response->assertStatus(422)
            ->assertJson([
                'dataArray' => [],
                'normalMessage' => '',
                'errorMessage' => '複製元LP構成IDは整数で指定してください。',
            ]);
    }
