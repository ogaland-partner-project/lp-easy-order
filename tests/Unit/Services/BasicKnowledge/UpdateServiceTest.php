...
class UpdateServiceTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        collect([
            ...
            factory(TBasicKnowledgeImage::class)->create(
                array_merge($row, [
                    'image_path' => sprintf(
                        '/storagee/lp_order/%d/basicknowledge/%d.jpg',
                        floor($row['id'] / 1000),
                        $row['id']
                    ),
    ...
    public function test_正常()
    {
        ...
        $this->service->execUpdate($basicKnowledgeId, $param);
        ...
        $this->assertNotEquals('TBasicKnowledgeFactory', $basicKnowledge->updated_pg);
        ...
        collect($param['details'])->each(function ($rows, $colIndex) use ($basicKnowledge) {
            ...
            collect($rows)->each(function ($row, $i) use ($colIndex, $basicKnowledge) {
                ...
                if (isset($row['id']) && !is_null($row['id'])) {
                    ...
                    $this->assertNotEquals('TBasicKnowledgeDetailFactory', $basicKnowledgeDetail->updated_pg);
                } else {
                    // - 追加レコードのチェック
                    $expect = array_merge($row, [
                        'basic_knowledge_id' => $basicKnowledge->id, 
                    ...
                    $this->assertNotNull($basicKnowledgeDetail->created_pg);
                    $this->assertNotNull($basicKnowledgeDetail->updated_pg);
                }
            });
        });
        ...
        collect($param['images'])->each(function ($row) use ($basicKnowledge) {
            ...
            if (isset($row['id']) && !is_null($row['id'])) {
                ...
                $this->assertNotEquals('TBasicKnowledgeImageFactory', $basicKnowledgeImage->updated_pg);
            } else {
        ...
        collect($param['urls'])->each(function ($row) use ($basicKnowledge) {
            if (isset($row['id']) && !is_null($row['id'])) {
                ...
                $this->assertArrayEquals($expect, $basicKnowledgeUrl, ['created_pg']);
                $this->assertNotEquals('TBasicKnowledgeUrlFactory', $basicKnowledgeUrl->updated_pg);
            } else {
                // - id指定のない情報は新規登録されていること
     ...
     * @return void
     */
    public function test_異常：基礎知識が非存在()
    {
     ...
     * @return void
     */
    public function test_異常：基礎知識が削除済()
    {
