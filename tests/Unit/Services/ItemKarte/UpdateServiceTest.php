...
class UpdateServiceTest extends TestCase
{
    ...
    public function test_正常()
    {
        ...
        $this->service->execUpdate($itemKarteId, $param);
        ...
        $this->assertNotEquals('TItemKarteFactory', $itemKarte->updated_pg);
        ...
        $updateRows->each(function ($row) use ($itemKarteId) {
            ...
            $this->assertArrayEquals($expect, $rawMaterial);
            $this->assertNotEquals('TRawMaterialFactory', $rawMaterial->updated_pg);
        });
        ...
        $createRows->each(function ($row) use ($itemKarteId) {
            ...
            $this->assertArrayEquals($expect, $rawMaterial, ['id']);
        ...
        // 他の商品カルテ情報：原材料情報は変化しないこと
        $afterOtherItemKartes = TItemKarte::find($otherItemKarteId);
        $this->assertArrayEquals($beforeOtherItemKartes, $afterOtherItemKartes);
        $afterOtherRawMaterials = TRawMaterial::where('item_karte_id', $otherItemKarteId)->orderBy('id')->get();
        $this->assertCount($beforeOtherRawMaterials->count(), $afterOtherRawMaterials);
        $beforeOtherRawMaterials->each(function ($beforeOtherRawMaterial, $i) use ($afterOtherRawMaterials) {
            $afterOtherRawMaterial = $afterOtherRawMaterials->skip($i)->first();
            $this->assertArrayEquals($beforeOtherRawMaterial, $afterOtherRawMaterial);
        });
    }
